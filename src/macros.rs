    ...
    ($page:ident, $constructor:path) => {
        ...
        assert!(
            deployutil::DEPLOYMENT_MAP
                .r()
                .exists(deployutil::DeploymentFileType::Source(&$page.src)),
            concat!(stringify!($page), " not found in deployment map")
        );
...
macro_rules! page_deploy {
    ($page:expr, $desc:expr) => {
        ...
        let deployment_path = binding
            .pop($crate::deployutil::DeploymentFileType::Source(
                &template_path,
            ))
            .unwrap_or_else(|| {
                panic!(
                    "Failed to find {} in deployment map",
                    template_path.display()
                )
            })
            .clone();
        drop(binding);
        $crate::deployutil::DEPLOYMENT_MAP
            .w()
            .mark(deployment_path.clone());
        let _ = $crate::deployutil::deploy_fn(&deployment_path, &$page.page, $desc)
            .expect(&format!("Failed to deploy {}", $desc));
    };
}
...
macro_rules! json_template {
    ...
    ($file:ident) => {{
        ...
        let file = std::fs::File::open(&path)
            .expect(format!("Failed to open `{}`", path.display()).as_str());
        serde_json::from_reader(file)
            .expect(format!("Failed to parse `{}`", path.display()).as_str())
    }};
    // --------------------------------------------------
...
macro_rules! url_relative_content {
    ($url:expr) => {{
        ...
        url_absolute
            .replace(concat!(env!("CARGO_MANIFEST_DIR"), "/content"), "")
            .to_string()
    }};
}
